...
> two restarts, not as seconds. `ulticleaner.clean.status` also needs the server driven below 20 TPS
> and then below 18, by repeated `fill` commands, and carries its own stated `blocked` exit if that
> cannot be reached.
...
| ulticleaner.clean.status | Sender holds `ulticleaner.clean`; the scheduled item and entity cleanup tasks are both enabled (shipped default); `tps.low-reduction: 60` and `tps.critical-reduction: 70` set before the server starts, both deliberately NOT the shipped 30 and 50 — a run left at the defaults cannot tell an interpolated value from the literal this row exists to prove is gone. **Both TPS thresholds start at their shipped values** (`tps.low-threshold: 18.0`, `tps.critical-threshold: 15.0`); the row raises them itself, at step 2, and not before. An idle server's one-minute average sits at 19.9-something for long stretches, so raising the low threshold to 20.0 up front would put the server in the "(Low)" band from the start and make step 1 unreachable | 1. On an idle server, with the thresholds still at their shipped 18.0/15.0, run `/clean status` and read the output. 2. Set `tps.low-threshold: 20.0` and `tps.critical-threshold: 18.0` — each its own `@Range` maximum, which is what makes the two warning bands reachable at all — save, and run `/ul reload UltiCleaner`. 3. Load the server until its TPS line reads "(Low)": from the console run `/minecraft:fill 0 -60 0 63 -53 63 minecraft:stone` (32768 blocks, the per-command maximum) repeatedly, checking `/clean status` between runs; stop as soon as the TPS line reads "(Low)" and read that same output. 4. Keep loading until the TPS line reads "(Critical)" and read that output. If after 30 `fill` commands the TPS line has never left "(Normal)", record `blocked` and say so — do not record `passed` on an unreached band | Step 1: chat prints (gloss) "=== Cleanup Status ===" (gold), then "Next item cleanup: S1 seconds" and "Next entity cleanup: S2 seconds" (both counting down from their own configured interval), then "Cleanup status: Idle" (gray) when no batch is running or "In progress..." (green) when one is, then the TPS line reading "(Normal)" — an idle server is at or above the shipped 18.0 threshold — and NO warning line after it. Step 3: the TPS line reads "(Low)" and is followed by exactly one further yellow warning line whose text ends "60%" — the configured `tps.low-reduction` — and NOT "30%". Step 4: the TPS line reads "(Critical)" and is followed by exactly one further red warning line whose text ends "70%" — the configured `tps.critical-reduction` — and NOT "50%", with no yellow line beside it (the critical branch replaces the low one, it does not add to it). Fixed, `UltiKits/UltiCleaner#21` | server | |
