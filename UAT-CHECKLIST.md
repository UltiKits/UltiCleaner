...
| ulticleaner.clean.status | Sender holds `ulticleaner.clean`; the scheduled item and entity cleanup tasks are both enabled (shipped default); set ALL FOUR of `tps.low-reduction: 60`, `tps.critical-reduction: 70`, `tps.low-threshold: 20.0` and `tps.critical-threshold: 18.0` before the server starts. The two reductions are deliberately NOT the shipped 30 and 50 — a run left at the defaults cannot tell an interpolated value from the literal this row exists to prove is gone. The two thresholds are each their own `@Range` maximum, so any dip below 20.00 TPS counts as low and any dip below 18.00 counts as critical, which is what makes the two warning bands reachable at all | 1. On an idle server, run `/clean status`. 2. Load the server until its TPS line reads "(Low)": from the console, run `/minecraft:fill 0 -60 0 63 -53 63 minecraft:stone` (32768 blocks, the per-command maximum) repeatedly, checking `/clean status` between runs; stop as soon as the TPS line reads "(Low)" and read that same output. 3. Keep loading until the TPS line reads "(Critical)" and read that output. If after 30 `fill` commands the TPS line has never left "(Normal)", record `blocked` and say so — do not record `passed` on an unreached band | Run 1: chat prints (gloss) "=== Cleanup Status ===" (gold), then "Next item cleanup: S1 seconds" and "Next entity cleanup: S2 seconds" (both counting down from their own configured interval), then "Cleanup status: Idle" (gray) when no batch is running or "In progress..." (green) when one is, then the TPS line reading "(Normal)", and NO warning line after it. Run 2: the TPS line reads "(Low)" and is followed by exactly one further yellow warning line whose text ends "60%" — the configured `tps.low-reduction` — and NOT "30%". Run 3: the TPS line reads "(Critical)" and is followed by exactly one further red warning line whose text ends "70%" — the configured `tps.critical-reduction` — and NOT "50%", with no yellow line beside it (the critical branch replaces the low one, it does not add to it). Fixed, `UltiKits/UltiCleaner#21` | server | |
