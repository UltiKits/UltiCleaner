...
| ulticleaner.clean.items.neg-in-progress | A large cleanup is already in progress: lower `batch.size` to 10 (default 50, so the window a small population gives is only ~1 tick — too narrow for a human or real-client executor to reliably hit) BEFORE the server starts (editing `config/cleaner.yml` while the server is running has no effect until a restart — `UltiCleaner#reloadSelf` never re-reads the file) and drop at least 200 non-whitelisted, unnamed, aged items, giving roughly 20 ticks (1 second) of processing time; run `/clean items` once and immediately proceed to the next step | Run `/clean items` again while the first batch is still processing | Chat line reads (gloss) "Cleanup already in progress, please wait..." (yellow); no second batch starts, no double-counting occurs | server | |
...
| ulticleaner.clean.chunks.neg-disabled | Sender holds `ulticleaner.clean`; `chunk.enabled: false` (the shipped default); at least one loaded chunk otherwise eligible for unload (far from every player, not force-loaded, not in use, holding no player) | Run `/clean chunks` | The chunk IS unloaded anyway, and chat reads (gloss) "Unloaded N idle chunks!" (green) with N at least 1 — `chunk.enabled` gates only the SCHEDULED task (`ChunkUnloadService#checkAndUnloadChunks`); `CleanCommand#cleanChunks`'s null-check on the (always-injected, unconditional `@Service`) `chunkUnloadService` field never actually refuses, and `forceUnloadChunks`/`collectChunksToUnload` never check `chunk.enabled` at all. Known product defect (checklist corrected from an earlier claim of a refusal message), `UltiKits/UltiCleaner#20` | server | |
...
| ID | Preconditions | Steps | Expected | Layer | Covers |
|---|---|---|---|---|---|
| ulticleaner.config.cleaner-yml | Fresh `config/cleaner.yml` at its shipped default | Load the file; confirm all 38 keys listed under `FEATURES.md`'s `## Configuration` section are present at their documented defaults; then RESTART THE SERVER COMPLETELY (not `/ul reload` — `CleanerService#reload` rebuilds caches from the existing config object, but `UltiCleaner#reloadSelf` never re-reads `config/cleaner.yml` from disk into that object, so editing the file while the server is running has no effect until a restart) and set `item.interval: 15` (default 300) before the restart, then repeat `ulticleaner.scheduled.item-tick` above, confirming the item cleanup now fires around 15 seconds later rather than 300. Do NOT vary `messages.prefix` expecting an observable effect — it has none (see its own `FEATURES.md` row and `UltiKits/UltiCleaner#18`) | All 38 keys present at their documented defaults before the change; after lowering `item.interval` to 15, the scheduled item cleanup fires (warning broadcasts, then removal) around 15 seconds later rather than 300, proving the lowered value took effect | server | |
